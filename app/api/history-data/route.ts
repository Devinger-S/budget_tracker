...
async function getYearHistoryData ( userId:string,year:number) {
    ...
    // [{
    //     month:0,
    //     expense: 3000 // sum of expense in that month
    //     income: 4000 // sum of income in that month
...
async function getMonthHistoryData(userId:string,year:number,month:number) {
    ...
    if (!result || result.length === 0) return "no data"
    const history: HistoryData[] = [];
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        let expense = 0
        let income = 0
        const day = result.find((object) => object.day === i )
        if (day) {
